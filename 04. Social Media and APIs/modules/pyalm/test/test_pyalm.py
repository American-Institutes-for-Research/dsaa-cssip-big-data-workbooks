...
class TestOnline(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_response_publication_date = datetime.datetime(2012, 1, 11, 0, 0)
...
class TestRetrieveALMs(TestOnline):
    def testGet(self):
        self.resp = pyalm.get_alm(self.test_doi, source = self.test_source, info ='detail')['articles'][0]
        #Basic Tests
        self.assertEqual(self.resp.title, self.test_response_title)
        self.assertEqual(self.resp.doi, self.test_response_doi)
        self.assertGreaterEqual(self.resp.viewed, self.test_response_views)
        self.assertEqual(type(self.resp.update_date), datetime.datetime)
        self.assertEqual(self.resp.issued, self.test_response_publication_date)
...
#         self.assertIsInstance(timepoints[0][0], datetime.datetime   )
    ...
    def testGetMultipleDOIs(self):
        self.resp = pyalm.get_alm(self.test_two_dois, info='summary')['articles']
...
class TestBasicElements(TestOffline):
    def testBasicElements(self):
        ...
        self.assertGreaterEqual(self.mock_resp.viewed, self.test_response_views)
        self.assertEqual(type(self.mock_resp.update_date), datetime.datetime)
        self.assertEqual(self.mock_resp.issued, self.test_response_publication_date)

class TestSourcesNumbers(TestOffline):
    def testSourcesNumbers(self):
        self.assertEqual(self.mock_resp.sources['twitter'].metrics.total, 12)
        self.assertEqual(self.mock_resp.sources['counter'].metrics.total, 36005)
        self.assertEqual(self.mock_resp.sources['crossref'].metrics.total, 9)
        self.assertEqual(self.mock_resp.sources['pmc'].metrics.total, 645)
...
class TestSourcesAttributes(TestOffline):
    def testSourcesAttributes(self):
        ...
        self.assertEqual(self.mock_resp.sources['mendeley'].update_date,
                                        datetime.datetime(2015, 7, 19, 12, 1, 24))

class TestSourcesMetrics(TestOffline):
    def testSourcesMetrics(self):
        self.assertEqual(self.mock_resp.sources['twitter'].metrics.comments, 12)
